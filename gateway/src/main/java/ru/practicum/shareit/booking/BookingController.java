...
public class BookingController {
	...
	public ResponseEntity<Object> changeBooking(@RequestHeader("X-Sharer-User-Id") int userId,
											@PathVariable long bookingId, @RequestParam boolean approved) {
		return bookingClient.changeBooking(userId, bookingId, approved);
	}

	@GetMapping("/owner")
	public ResponseEntity<Object> getAllBookingOwner(@RequestHeader("X-Sharer-User-Id") int userId,
                                                     @RequestParam(name = "state", defaultValue = "all") String stateParam,
                                                     //@PositiveOrZero @RequestParam(required = false) Integer from,
                                                     //@Positive @RequestParam(required = false) Integer size) {
                                                     @PositiveOrZero @RequestParam(name = "from", defaultValue = "0") Integer from,
